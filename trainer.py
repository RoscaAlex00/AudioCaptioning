...
class BARTAACTrainer(Trainer):
    ...
    def caption_evaluate(self, eval_dataset, tokenizer, generation_mode='beam'):
        ...
        with torch.no_grad():
            for step, inputs in enumerate(tqdm(eval_dataloader)):
                ...
                all_preds.append(F.pad(outputs, (0, 512 - outputs.size(1)), 'constant',
                                       1).cpu())  # Pad with pad token for concatenation
        ...
        print(all_preds)
        metrics, all_gt_captions, all_pred_captions = aac_metrics(
            {'predictions': all_preds, 'label_ids': all_labels, 'filenames': all_filenames}, tokenizer)
    ...
    def caption_infer(self, test_dataset, tokenizer, generation_mode='beam'):
        ...
        for i_pred in range(all_preds.shape[0]):
            ...
            all_pred_caps.append(
                pred_.replace('<|pad|>', '').replace('<|endoftext|>', '').replace('</s>', '').replace('<s>',
                                                                                                      '').replace(
                    '<pad>', ''))
