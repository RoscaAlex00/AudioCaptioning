...
def main(config):
    ...
    # Trainer
    trainer = BARTAACTrainer(model, args=training_args, data_collator=default_data_collator, train_dataset=data_train,
    ...
    if settings['workflow']['evaluate'] or settings['workflow']['infer']:
        ...
        if settings['lm']['eval_model'] == 'checkpoint':  # Specific checkpoint
            model.load_state_dict(
                torch.load(out_dir + '/checkpoint-' + str(settings['lm']['eval_checkpoint']) + '/pytorch_model.bin',
                           map_location=device))
