...
def aac_metrics(outputs, tokenizer):
    ...
    for i_ex in range(outputs['predictions'].shape[0]):
        ...
        gt_captions.append(
            gt_.replace('<|pad|>', '').replace('<|endoftext|>', '').replace('</s>', '').replace('<s>', '').replace(
                '<pad>', ''))
        pred_ = tokenizer.decode(outputs['predictions'][i_ex, :])
        pred_captions.append(
            pred_.replace('<|pad|>', '').replace('<|endoftext|>', '').replace('</s>', '').replace('<s>', '').replace(
                '<pad>', ''))

        # Group for COCO metrics
        if i_ex == len(outputs['filenames']) - 1 or outputs['filenames'][i_ex + 1] != outputs['filenames'][
            i_ex]:  # Last example for current audio
